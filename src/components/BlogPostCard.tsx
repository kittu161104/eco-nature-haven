    ...
    <article className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-sm hover:shadow-md transition-shadow duration-300">
      ...
      <div className="p-5">
        ...
        <Link to={`/blog/category/${post.category.toLowerCase()}`}>
          <span className="inline-block px-2 py-1 bg-eco-100 dark:bg-eco-900 text-eco-800 dark:text-eco-300 text-xs font-medium rounded mb-3">
            {post.category}
          </span>
        </Link>
        ...
        <Link to={`/blog/${post.id}`}>
          <h3 className="text-xl font-serif font-bold text-gray-900 dark:text-white mb-2 hover:text-eco-700 dark:hover:text-eco-400 transition-colors">
            {post.title}
          </h3>
        </Link>
